...
interface IPDMStaking {
	...
	event TotalPDMStakedUpdated(uint256 _totalPDMStaked);
	...
	function pdmToken() external view returns (IERC20Upgradeable);

	function stake(uint256 _PDMamount) external;

	function unstake(uint256 _PDMamount) external;

	function increaseFee_Asset(address _asset, uint256 _AssetFee) external;

	function increaseFee_DebtToken(uint256 _PDMFee) external;
