...
contract FeeCollector is IFeeCollector, UUPSUpgradeable, OwnableUpgradeable, Addresses {
	...
	uint256 public constant MIN_FEE_FRACTION = 1 ether; // All fee charged at the time of borrowing
	uint256 public constant FEE_EXPIRATION_SECONDS = 0;

	// State ------------------------------------------------------------------------------------------------------------

	mapping(address => mapping(address => FeeRecord)) public feeRecords; // borrower -> asset -> fees

	bool public constant routeToPDMStaking = false; // if true, collected fees go to stakers; if false, to the treasury
	...
	function getProtocolRevenueDestination() public view override returns (address) {
		return _routeToPDMStaking() ? pdmStaking : treasuryAddress;
	}
	...
	/**
	 * Transfers collected (debt token) fees to either the treasury or the PDMStaking contract, depending on a flag.
	 */
	function _collectFee(address _borrower, address _asset, uint256 _feeAmount) internal {
		if (_feeAmount != 0) {
			...
			if (_routeToPDMStaking()) {
				IPDMStaking(pdmStaking).increaseFee_DebtToken(_feeAmount);
			}
	 ...
	 */
	function _routeToPDMStaking() internal view virtual returns (bool) {
		return routeToPDMStaking;
	}
