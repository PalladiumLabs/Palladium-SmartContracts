...
interface IStabilityPool is IDeposit {
	...
	event PDMPaidToDepositor(address indexed _depositor, uint256 _PDM);
	 ...
	 * - Sends depositor's accumulated gains (PDM, assets) to depositor
	 */
	 ...
	 * ---
	 * - Triggers a PDM issuance, based on time passed since the last issuance. The PDM issuance is shared between *all* depositors.
	 * - Sends all depositor's accumulated gains (PDM, assets) to depositor
	 ...
	 */
	function getDepositorPDMGain(address _depositor) external view returns (uint256);
